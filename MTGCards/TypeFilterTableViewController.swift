...
class TypeFilterTableViewController: UITableViewController {
    ...
    func getTypes(){
        let fetchRequest: NSFetchRequest<CDType> = CDType.fetchRequest()
        ...
        do {
            ...
            //print(c.count)
            types = Array(Set(c.map({$0.type!}))).sorted()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let checked = filterController?.cardListView?.filters.types.contains(types[indexPath.row]){
            if checked {
