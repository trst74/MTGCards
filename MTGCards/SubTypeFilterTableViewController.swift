...
class SubTypeFilterTableViewController: UITableViewController {
    ...
    func getTypes(){
        let fetchRequest: NSFetchRequest<CDSubType> = CDSubType.fetchRequest()
        ...
        do {
            ...
            //print(c.count)
            types = Array(Set(c.map({$0.subtype!}))).sorted()
    ...
    override func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let checked = filterController?.cardListView?.filters.subTypes.contains(types[indexPath.row]){
            if checked {
