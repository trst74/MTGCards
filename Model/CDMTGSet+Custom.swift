...
extension CDMTGSet{
    ...
    func updateCDSetFromSet(source: MTGSet, inContext context: NSManagedObjectContext) {
        self.name = source.name
        self.code = source.code
        self.magicCardsInfoCode = source.magicCardsInfoCode
        self.releaseDate = source.releaseDate
        ...
        //cards
        for card in source.cards! {
            if let existingCard = CardDatabaseHelper.getCard(cardId: card.id!) {
                existingCard.updateCardFromCard(source: card, inContext: context)
            } else {
                self.addToCards(CDCard.init(from: card, inContext: context))
            }
